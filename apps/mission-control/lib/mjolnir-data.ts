...
export type FitnessAlertSeverity = "critical" | "warning" | "info";
...
export const normalizeDurationSeconds = (value: unknown): number | null => {
  const parsed = parseNumber(value);
...
export const getPath = (record: unknown, path: string): unknown => {
...
export const pickNumber = (record: Record<string, unknown> | null | undefined, paths: string[]) => {
...
export const pickString = (record: Record<string, unknown> | null | undefined, paths: string[]) => {
  if (!record) return null;
  for (const path of paths) {
    const value = getPath(record, path);
...
export async function fetchFitnessData(): Promise<{ payload: FitnessResponse; cached: boolean }> {
  const now = Date.now();

...
def _build_prediction_record(
    *,
    ...
    return {
        ...
        "calibrated_confidence": contract_fields.get("calibrated_confidence"),
        "strategy_family": contract_fields.get("strategy_family"),
        "opportunity_score": contract_fields.get("opportunity_score"),
        "downside_risk": contract_fields.get("downside_risk"),
        "canonical_horizon_days": contract_fields.get("canonical_horizon_days"),
        "score_mapping_version": contract_fields.get("score_mapping_version"),
        ...
        "feature_summary": contract_fields.get("feature_summary"),
        "benchmark_context": contract_fields.get("benchmark_context"),
...
def _prefer_cache_backed_symbols(
    ...
    *,
    ...
    if len(cached) >= min_cache_only_symbols:
        ...
        return (
            selected,
            f"Degraded fallback: scanning {len(selected)} cache-backed Dip Buyer names while live market data recovers.",
        )

    ordered = [*cached, *uncached][: len(symbols)]
    return (
        ordered,
        f"Degraded fallback: only {len(cached)} cache-backed Dip Buyer names were available, so they were prioritized first.",
    )
...
def _deterministic_universe(
    ...
    *,
    ...
    if hasattr(advisor, "screener"):
        ...
        return symbols, 0, None, fallback_line
        ...
        serialized.append(
            {
                ...
                "credit_veto": bool(record.get("credit_veto", False)),

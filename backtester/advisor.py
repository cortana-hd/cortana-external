...
class TradingAdvisor:
    ...
    def _prediction_risk_label(
        *,
        ...
        trade_quality = TradingAdvisor._prediction_to_float(
            rec.get("trade_quality_score", context.get("trade_quality_score"))
        )
        uncertainty_pct = TradingAdvisor._prediction_to_float(
            rec.get("uncertainty_pct", context.get("uncertainty_pct"))
        )
        adverse_regime_score = TradingAdvisor._prediction_to_float(
            rec.get(
                "adverse_regime_score",
                context.get("adverse_regime_score", (context.get("adverse_regime") or {}).get("score")),
            )
        )
        exit_risk_score = TradingAdvisor._prediction_to_float(
            rec.get("exit_risk_score", (context.get("exit_risk") or {}).get("score"))
        )
        ...
        if any(
            value is not None
            for value in (trade_quality, uncertainty_pct, adverse_regime_score, exit_risk_score)
        ):
            return "medium"
        return "unknown"

    @staticmethod
    def _prediction_entry_plan_ref(
        *,
        strategy: str,
        recommendation: Optional[Dict] = None,
        analysis: Optional[Dict] = None,
    ...
    @staticmethod
    def _prediction_execution_policy_ref(
        *,
        recommendation: Optional[Dict] = None,
        analysis: Optional[Dict] = None,
        execution_overlay: Optional[Dict] = None,
    ) -> str | None:
        rec = recommendation if isinstance(recommendation, dict) else {}
        context = analysis if isinstance(analysis, dict) else {}
        overlay = execution_overlay if isinstance(execution_overlay, dict) else {}
        explicit = str(
            rec.get("execution_policy_ref")
            or context.get("execution_policy_ref")
            or ""
        ).strip()
        if explicit:
            return explicit
    ...
    def build_prediction_contract_context(
        ...
        *,
        ...
        rec = recommendation if isinstance(recommendation, dict) else {}
        context = analysis if isinstance(analysis, dict) else {}
        effective_confidence = cls._prediction_to_float(
            rec.get("effective_confidence", rec.get("confidence", context.get("effective_confidence", context.get("confidence"))))
        )
        normalized_breadth_state = str(
            breadth_state
            or rec.get("breadth_state")
            or context.get("breadth_state")
            or ""
        ).strip() or None
        market_regime = str(
        ...
        return {
            ...
            "entry_plan_ref": cls._prediction_entry_plan_ref(
                strategy=strategy,
                recommendation=rec,
                analysis=context,
            ),
            "execution_policy_ref": cls._prediction_execution_policy_ref(
                recommendation=rec,
                analysis=context,
